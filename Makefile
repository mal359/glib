...
DATA =  $(configinclude_DATA) $(m4data_DATA) $(pkgconfig_DATA)
...
glib.pc: $(top_builddir)/config.status glib.pc.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
gmodule.pc: $(top_builddir)/config.status gmodule.pc.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
gthread.pc: $(top_builddir)/config.status gthread.pc.in
	cd $(top_builddir) && CONFIG_FILES=$@ CONFIG_HEADERS= $(SHELL) ./config.status
...
installdirs-am:
	$(mkinstalldirs)  $(DESTDIR)$(libdir) $(DESTDIR)$(bindir) \
		...
		$(DESTDIR)$(pkgconfigdir) $(DESTDIR)$(glibincludedir)
...
install-pkgconfigDATA uninstall-glibincludeHEADERS \
install-glibincludeHEADERS install-data-recursive \
